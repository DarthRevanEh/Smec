...
class Fighter_Cannon
	...
	if player_colour=black then % player is black
	    right(pic_pose_run):=Pic.FileNew("cannon_right_run.bmp")
	    right(pic_pose_jump1):=Pic.FileNew("cannon_right_jump.bmp")
	    right(pic_pose_hit):=Pic.FileNew("cannon_right_hit.bmp")
	    right(pic_pose_ground_hit):=Pic.FileNew("cannon_right_ground_hit.bmp")
	    right(pic_pose_air_forward_hit):=Pic.FileNew("cannon_right_air_forward_hit.bmp")
	    right(pic_pose_air_down_hit):=Pic.FileNew("cannon_right_air_down_hit.bmp")
	    right(pic_pose_air_up_hit):=Pic.FileNew("cannon_right_air_up_hit.bmp")
	    right(pic_pose_air_back_hit):=Pic.FileNew("cannon_right_air_back_hit.bmp")
	    right(pic_pose_air_neutral_hit):=Pic.FileNew("cannon_right_air_neutral_hit.bmp")
	    
	    left(pic_pose_run):=Pic.FileNew("cannon_left_run.bmp")
	    left(pic_pose_jump1):=Pic.FileNew("cannon_left_jump.bmp")
	    left(pic_pose_hit):=Pic.FileNew("cannon_left_hit.bmp")
	    left(pic_pose_ground_hit):=Pic.FileNew("cannon_left_ground_hit.bmp")
	    left(pic_pose_air_forward_hit):=Pic.FileNew("cannon_left_air_forward_hit.bmp")
	    left(pic_pose_air_down_hit):=Pic.FileNew("cannon_left_air_down_hit.bmp")
	    left(pic_pose_air_up_hit):=Pic.FileNew("cannon_left_air_up_hit.bmp")
	    left(pic_pose_air_back_hit):=Pic.FileNew("cannon_left_air_back_hit.bmp")
	    left(pic_pose_air_neutral_hit):=Pic.FileNew("cannon_left_air_neutral_hit.bmp")
	    
	elsif player_colour=green then % player is green
	    right(pic_pose_run):=Pic.FileNew("cannon_right_run_g.bmp")
	    right(pic_pose_jump1):=Pic.FileNew("cannon_right_jump_g.bmp")
	    right(pic_pose_hit):=Pic.FileNew("cannon_right_hit_g.bmp")
	    right(pic_pose_ground_hit):=Pic.FileNew("cannon_right_ground_hit_g.bmp")
	    right(pic_pose_air_forward_hit):=Pic.FileNew("cannon_right_air_forward_hit_g.bmp")
	    right(pic_pose_air_down_hit):=Pic.FileNew("cannon_right_air_down_hit_g.bmp")
	    right(pic_pose_air_up_hit):=Pic.FileNew("cannon_right_air_up_hit_g.bmp")
	    right(pic_pose_air_back_hit):=Pic.FileNew("cannon_right_air_back_hit_g.bmp")
	    right(pic_pose_air_neutral_hit):=Pic.FileNew("cannon_right_air_neutral_hit_g.bmp")
	    
	    
	    left(pic_pose_run):=Pic.FileNew("cannon_left_run_g.bmp")
	    left(pic_pose_jump1):=Pic.FileNew("cannon_left_jump_g.bmp")
	    left(pic_pose_hit):=Pic.FileNew("cannon_left_hit_g.bmp")
	    left(pic_pose_ground_hit):=Pic.FileNew("cannon_left_ground_hit_g.bmp")
	    left(pic_pose_air_forward_hit):=Pic.FileNew("cannon_left_air_forward_hit_g.bmp")
	    left(pic_pose_air_down_hit):=Pic.FileNew("cannon_left_air_down_hit_g.bmp")
	    left(pic_pose_air_up_hit):=Pic.FileNew("cannon_left_air_up_hit_g.bmp")
	    left(pic_pose_air_back_hit):=Pic.FileNew("cannon_left_air_back_hit_g.bmp")
	    left(pic_pose_air_neutral_hit):=Pic.FileNew("cannon_left_air_neutral_hit_g.bmp")
	end if
	...
	if (can_hit=true) and (is_hitting=false) and (in_air=true) then
	    ...
	    % which way they are attacking
	    attack_hitbox_y:=player_y + (70) % same in both directions
	    if facing_right= false then
	...
	if (can_hit=true) and (is_hitting=false) and (in_air=true) then
	    ...
	    % which way they are attacking
	    attack_hitbox_y:=player_y + (5)% same in both directions
	    if facing_right= false then
	...
	if (can_hit=true) and (is_hitting=false) and (in_air=true) then
	    ...
	    attack_velo_x_multiplier:=0.2
	    attack_velo_y_multiplier:=1.5
	    attack_hit_stun_multiplier:=0.5
	    current_attack:=pic_pose_air_up_hit
	    % which way they are attacking
	    attack_hitbox_y:=player_y + pic_height + (48)% same in both directions
	    if facing_right= false then
		current_image:=left(pic_pose_air_up_hit)
		attack_hitbox_x:=player_x + 6
		attack_velocity_x:=-1
	    elsif facing_right=true then
		current_image:=right(pic_pose_air_up_hit)
		attack_hitbox_x:=player_x + pic_width - (6)
		attack_velocity_x:=1
	    end if
	end if
    end doAirUpHit
	...
	if (can_hit=true) and (is_hitting=false) and (in_air=true) then
		...
		attack_hitbox_x:=player_x + pic_width + 15
		attack_velocity_x:=1
	    elsif facing_right=true then
		current_image:=right(pic_pose_air_back_hit)
		attack_hitbox_x:=player_x - 15
		attack_velocity_x:=-1
	    end if
	end if
    end doAirBackHit
	...
	if (can_hit=true) and (is_hitting=false) and (in_air=true) then
	    ...
	    attack_hit_stun:=10
	    attack_velocity_y:=15
	    attack_velo_x_multiplier:=4
	    attack_velo_y_multiplier:=1
	    attack_hit_stun_multiplier:=0.5
	    current_attack:=pic_pose_air_neutral_hit
	    % which way they are attacking
	    attack_hitbox_y:=player_y + (30) % same in both directions
	    if facing_right= false then
    ...
    % TEST FOR THE INDIVDUAL MOVES
		if current_attack=pic_pose_ground_hit then % you don't need to update this move, the hitbox does not change
		    ...
		    attack_hitbox_y:=player_y + pic_height + (48) 
		    if (facing_right=false) then
			attack_hitbox_x:=player_x  + (7)
		    elsif (facing_right=true) then
			attack_hitbox_x:=player_x  + pic_width - (7)
		    end if
		    
		    
		elsif current_attack=pic_pose_air_back_hit then % BACK AIR
		    attack_hitbox_y:=player_y + (65)
		    if (facing_right=false) then
			attack_hitbox_x:=player_x + pic_width + 15
		    elsif (facing_right=true) then
			attack_hitbox_x:=player_x - 15
	...
	if (current_image=right(pic_pose_jump1)) or (current_image=left(pic_pose_jump1)) or (current_image=right(pic_pose_run)) or (current_image=left(pic_pose_run)) then
	    Pic.Draw(current_image, player_x, player_y, picMerge)
	    
	elsif (current_image=right(pic_pose_hit)) or (current_image=left(pic_pose_hit)) then
	    Pic.Draw(current_image, player_x - (15), player_y, picMerge)
	    
	elsif (current_image=right(pic_pose_ground_hit)) then 
	    Pic.Draw(current_image, player_x, player_y, picMerge)
	elsif (current_image=left(pic_pose_ground_hit)) then 
	    Pic.Draw(current_image, player_x - (30), player_y, picMerge)
	    
	elsif (current_image=right(pic_pose_air_neutral_hit)) or (current_image=left(pic_pose_air_neutral_hit)) or (current_image=right(pic_pose_air_forward_hit)) or (current_image=left(pic_pose_air_forward_hit)) or (current_image=right(pic_pose_air_back_hit)) or (current_image=left(pic_pose_air_back_hit)) then
	    Pic.Draw(current_image, player_x - (30), player_y, picMerge)
