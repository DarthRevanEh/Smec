...
class Fighter_Dante
    ...
    body procedure initialize (left_key_, right_key_, up_key_, down_key_, attack_key_:char, stage_x_left_, stage_x_right_, stage_y_, player_number, lives_, player_colour_:int, other_player_:^Fighter)
        ...
        if player_colour=black then % player is black
            right(pic_pose_run):=Pic.FileNew("dante_right_run.bmp")
            right(pic_pose_jump1):=Pic.FileNew("dante_right_jump1.bmp")
            right(pic_pose_jump2):=Pic.FileNew("dante_right_jump2.bmp")
            right(pic_pose_hit):=Pic.FileNew("dante_right_hit.bmp")
            right(pic_pose_ground_hit):=Pic.FileNew("dante_right_ground_hit.bmp")
            right(pic_pose_air_forward_hit):=Pic.FileNew("dante_right_air_forward_hit.bmp")
            right(pic_pose_air_down_hit):=Pic.FileNew("dante_right_air_down_hit.bmp")
            right(pic_pose_air_up_hit):=Pic.FileNew("dante_right_air_up_hit.bmp")
            right(pic_pose_air_back_hit):=Pic.FileNew("dante_right_air_back_hit.bmp")
            right(pic_pose_air_neutral_hit):=Pic.FileNew("dante_right_air_neutral_hit.bmp")
            
            left(pic_pose_run):=Pic.FileNew("dante_left_run.bmp")
            left(pic_pose_jump1):=Pic.FileNew("dante_left_jump1.bmp")
            left(pic_pose_jump2):=Pic.FileNew("dante_left_jump2.bmp")
            left(pic_pose_hit):=Pic.FileNew("dante_left_hit.bmp")
            left(pic_pose_ground_hit):=Pic.FileNew("dante_left_ground_hit.bmp")
            left(pic_pose_air_forward_hit):=Pic.FileNew("dante_left_air_forward_hit.bmp")
            left(pic_pose_air_down_hit):=Pic.FileNew("dante_left_air_down_hit.bmp")
            left(pic_pose_air_up_hit):=Pic.FileNew("dante_left_air_up_hit.bmp")
            left(pic_pose_air_back_hit):=Pic.FileNew("dante_left_air_back_hit.bmp")
            left(pic_pose_air_neutral_hit):=Pic.FileNew("dante_left_air_neutral_hit.bmp")
            
        elsif player_colour=red then % player is red
            right(pic_pose_run):=Pic.FileNew("dante_right_run_r.bmp")
            right(pic_pose_jump1):=Pic.FileNew("dante_right_jump1_r.bmp")
            right(pic_pose_jump2):=Pic.FileNew("dante_right_jump2_r.bmp")
            right(pic_pose_hit):=Pic.FileNew("dante_right_hit_r.bmp")
            right(pic_pose_ground_hit):=Pic.FileNew("dante_right_ground_hit_r.bmp")
            right(pic_pose_air_forward_hit):=Pic.FileNew("dante_right_air_forward_hit_r.bmp")
            right(pic_pose_air_down_hit):=Pic.FileNew("dante_right_air_down_hit_r.bmp")
            right(pic_pose_air_up_hit):=Pic.FileNew("dante_right_air_up_hit_r.bmp")
            right(pic_pose_air_back_hit):=Pic.FileNew("dante_right_air_back_hit_r.bmp")
            right(pic_pose_air_neutral_hit):=Pic.FileNew("dante_right_air_neutral_hit_r.bmp")
            
            left(pic_pose_run):=Pic.FileNew("dante_left_run_r.bmp")
            left(pic_pose_jump1):=Pic.FileNew("dante_left_jump1_r.bmp")
            left(pic_pose_jump2):=Pic.FileNew("dante_left_jump2_r.bmp")
            left(pic_pose_hit):=Pic.FileNew("dante_left_hit_r.bmp")
            left(pic_pose_ground_hit):=Pic.FileNew("dante_left_ground_hit_r.bmp")
            left(pic_pose_air_forward_hit):=Pic.FileNew("dante_left_air_forward_hit_r.bmp")
            left(pic_pose_air_down_hit):=Pic.FileNew("dante_left_air_down_hit_r.bmp")
            left(pic_pose_air_up_hit):=Pic.FileNew("dante_left_air_up_hit_r.bmp")
            left(pic_pose_air_back_hit):=Pic.FileNew("dante_left_air_back_hit_r.bmp")
            left(pic_pose_air_neutral_hit):=Pic.FileNew("dante_left_air_neutral_hit_r.bmp")
        end if
        ...
        player_y:=stage_y 
        max_right_velocity:=11
        max_left_velocity:=-11
    ...
    body procedure doGroundForwardHit
        if (can_hit=true) and (is_hitting=false) and (in_air=false) then
            ...
            moveable:=false
            is_hitting_timer:=10 
            hit_cooldown_timer:=15
            attack_hit_stun:=9
            attack_velocity_y:=12
            attack_velo_x_multiplier:=0
            attack_velo_y_multiplier:=0.2
            attack_hit_stun_multiplier:=1
            current_attack:=pic_pose_ground_hit
            % which way they are attacking
            attack_hitbox_y:=player_y + (65) % % same in both directions
            if facing_right= false then
                current_image:=left(pic_pose_ground_hit)
                attack_hitbox_x:=player_x - 3
                attack_velocity_x:=-2
            elsif facing_right=true then
                current_image:=right(pic_pose_ground_hit)
                attack_hitbox_x:=player_x + pic_width + 3
                attack_velocity_x:=2
            end if
        end if
    ...
    body procedure doAirDownHit
        if (can_hit=true) and (is_hitting=false) and (in_air=true) then
            ...
            moveable:=true
            is_hitting_timer:=15 
            hit_cooldown_timer:=8
            attack_hit_stun:=12
            attack_velocity_y:=-13
            attack_velo_x_multiplier:=0.5
            attack_velo_y_multiplier:=2
            attack_hit_stun_multiplier:=3
    ...
    body procedure doAirUpHit
        if (can_hit=true) and (is_hitting=false) and (in_air=true) then
            ...
            hit_cooldown_timer:=8
            attack_hit_stun:=15
            attack_velocity_y:=16
            attack_velo_x_multiplier:=0.2
            attack_velo_y_multiplier:=0.5
            attack_hit_stun_multiplier:=1.5
            current_attack:=pic_pose_air_up_hit
            % which way they are attacking
            attack_hitbox_y:=player_y + pic_height% same in both directions
            if facing_right= false then
    ...
    body procedure doAirBackHit
        if (can_hit=true) and (is_hitting=false) and (in_air=true) then
            ...
            attack_hit_stun:=4
            attack_velocity_y:=3
            attack_velo_x_multiplier:=4.5
            attack_velo_y_multiplier:=4.5
            attack_hit_stun_multiplier:=1.5
    ...
    body procedure doAirNeutralHit
        if (can_hit=true) and (is_hitting=false) and (in_air=true) then
            ...
            moveable:=true
            is_hitting_timer:=12 
            hit_cooldown_timer:=2
            attack_hit_stun:=15
            attack_velocity_y:=22
            attack_velo_x_multiplier:=0.2
            attack_velo_y_multiplier:=0
            attack_hit_stun_multiplier:=1.5
    ...
    body procedure updateAirMoveHitboxes
        ...
        if current_attack=pic_pose_ground_hit then % you don't need to update this move, the hitbox does not change
            ...
            attack_hitbox_y:=player_y + pic_height
            if (facing_right=false) then
                attack_hitbox_x:=player_x
            elsif (facing_right=true) then
                attack_hitbox_x:=player_x  + pic_width
    ...
    body procedure updateFighter
        if (is_dead=true) then % test if the player is dead
            ...
            % SPECIAL POWER -> DANTE'S FLOAT. WHEN DANTE IS TRAVELLING DOWNWARDS IN THE AIR (AND HE CAN MOVE) HE FALLS SLOWLY, AND WILL NOT FALL ANY FASTER IF NO INPUT IS GIVEN
            if (current_image=right(pic_pose_jump2)) or (current_image=left(pic_pose_jump2)) then % he travelling downwards, make him float
                if (velocity_y>=-3) and (velocity_y<=-1) then
                    velocity_y:=-3
                else % do nothing, keep his current speed
                end if
            else % update his gravity as normal
            ...
            if (in_air=true) and (moveable=true) and (is_hitting=false) then
                if velocity_y>=0 then
                    if facing_right=false then % player is going up. Switch to jump 1 (jumping)
                        current_image:=left(pic_pose_jump1) % the player is facing left
                    elsif facing_right then
                        current_image:=right(pic_pose_jump1)
                    end if
                elsif velocity_y<0 then % the player is going down. Switch to jump 2 (floating)
                    if facing_right=false then
                        current_image:=left(pic_pose_jump2) % the player is facing left
                    elsif facing_right then
                        current_image:=right(pic_pose_jump2)
                    end if
            ...
            if (is_hitting=true) and (can_hit=false) then
                ...
                if (attack_hitbox_x >= other_player_x) and (attack_hitbox_x<=other_player_x+pic_width) and (attack_hitbox_y>=other_player_y) and (attack_hitbox_y<=other_player_y+pic_height) then % the other player is in this player's hitbox!
                    Fighter(other_player).onHit(attack_velocity_x, attack_velocity_y, attack_hit_stun, attack_velo_x_multiplier, attack_velo_y_multiplier, attack_hit_stun_multiplier) % hit the other player
                end if
    ...
    body procedure drawFighter
        if (current_image=right(pic_pose_jump1)) then
            Pic.Draw(current_image, player_x - (30), player_y, picMerge)
        elsif (current_image=left(pic_pose_jump1)) then
            Pic.Draw(current_image, player_x, player_y, picMerge)
            
        elsif (current_image=right(pic_pose_run)) or (current_image=right(pic_pose_air_neutral_hit)) or (current_image=right(pic_pose_air_down_hit)) or (current_image=right(pic_pose_air_back_hit)) or (current_image=right(pic_pose_air_forward_hit)) then
            Pic.Draw(current_image, player_x - (20), player_y, picMerge)
        elsif (current_image=left(pic_pose_run)) or (current_image=left(pic_pose_air_neutral_hit)) or (current_image=left(pic_pose_air_down_hit)) or (current_image=left(pic_pose_air_back_hit)) or (current_image=left(pic_pose_air_forward_hit)) then
            Pic.Draw(current_image, player_x - (10), player_y, picMerge)
            
        elsif (current_image=right(pic_pose_jump2)) or (current_image=right(pic_pose_ground_hit)) or (current_image=right(pic_pose_air_up_hit)) then
            Pic.Draw(current_image, player_x - (25), player_y, picMerge)
        elsif (current_image=left(pic_pose_jump2)) or (current_image=left(pic_pose_ground_hit)) or (current_image=left(pic_pose_air_up_hit)) then
            Pic.Draw(current_image, player_x - (5), player_y, picMerge)
            
        else % only the getting hit image is draw directly in the center
            Pic.Draw(current_image, player_x, player_y, picMerge)
        end if
    end drawFighter
